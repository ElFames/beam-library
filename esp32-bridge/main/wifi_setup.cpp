...
void on_wifi_sta_got_ip(void*, esp_event_base_t, int32_t, void* event_data) {
    ...
    // móvil/desktop): a partir de aquí el puente reparte internet de forma
    ...
    ESP_LOGI(TAG, "NAT activado: %s ahora reparte internet a su red", BRIDGE_AP_SSID);
#else
...
void start_ap() {
    ...
    strncpy(reinterpret_cast<char*>(ap_config.ap.ssid), BRIDGE_AP_SSID, sizeof(ap_config.ap.ssid));
    ap_config.ap.ssid_len = strlen(BRIDGE_AP_SSID);
    strncpy(reinterpret_cast<char*>(ap_config.ap.password), BRIDGE_AP_PASSWORD, sizeof(ap_config.ap.password));

    // Máxima seguridad de red pedida para este dispositivo (PROJECT.md §3):
    // WPA3-SAE en vez de WPA2, con Protected Management Frames obligatorio
    // (802.11w — cierra el vector de deauth que sí funciona contra la
    // mayoría de APs domésticos, que siguen en WPA2 sin PMF).
    //
    // OJO al compilar: confirma `WIFI_AUTH_WPA3_PSK` y los campos de
    // `pmf_cfg` contra la versión exacta de ESP-IDF que uses — no lo demos
    // por cerrado sin comprobarlo contra la documentación/ejemplos de esa
    // versión concreta antes de flashear una unidad real.
...
void save_sta_credentials(const std::string& ssid, const std::string& password) {
    ...
    nvs_commit(handle);
    nvs_close(handle);
    ESP_LOGI(TAG, "Credenciales de la WiFi de casa guardadas; reinicia el puente para aplicarlas");
}
...
void start() {
    ...
    if (has_sta) {
        ...
        esp_wifi_connect();
        ESP_LOGI(TAG, "Modo AP+STA: %s (propia) + %s (internet)", BRIDGE_AP_SSID, ssid.c_str());
    } else {
        ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_AP));
        start_ap();
        ESP_ERROR_CHECK(esp_wifi_start());
        ESP_LOGI(TAG, "Modo AP puro: %s (sin WiFi de casa configurada todavía)", BRIDGE_AP_SSID);
